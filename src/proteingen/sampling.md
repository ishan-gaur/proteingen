...
Sampling algorithms that generate sequences from TransitionModel (or guided TAG/DEG) instances.

## Dependencies

- [generative_modeling.md](generative_modeling.md) — calls `get_log_probs` on `TransitionModel` instances
- [guide.md](guide.md) — TAG/DEG are TransitionModel subclasses, work transparently with these samplers
...
- `Integrator` — `Callable[[TransitionModel, List[str]], List[str]]`
...
- **Device handling**: `sample_any_order_ancestral` and `sample_linear_interpolation` move input to `model.device`, return on original device (or as strings).
