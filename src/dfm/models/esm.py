...
class ESM3(TransitionModelWithEmbedding):
    ...
    def __init__(self, esm3_checkpoint: str = "esm3-open"):
        ...
        esm3 = _ESM3.from_pretrained(esm3_checkpoint, device=torch.device("cpu")).float().eval()
    ...
    def collate_observations(self, seq_SP: torch.LongTensor, observations: dict) -> dict:
        """Tile cached structure to match batch size."""
        B = seq_SP.shape[0]
        return {
            "structure_tokens": observations["structure_tokens"].unsqueeze(0).expand(B, -1),
            "coordinates": observations["coordinates"].unsqueeze(0).expand(B, -1, -1, -1),
        }
    ...
    def _non_sequence_embedding(
        ...
        if structure_tokens is None:
            ...
            structure_tokens.masked_fill_(seq_SP == C.SEQUENCE_BOS_TOKEN, C.STRUCTURE_BOS_TOKEN)
            structure_tokens.masked_fill_(seq_SP == C.SEQUENCE_PAD_TOKEN, C.STRUCTURE_PAD_TOKEN)
            structure_tokens.masked_fill_(seq_SP == C.SEQUENCE_EOS_TOKEN, C.STRUCTURE_EOS_TOKEN)
            structure_tokens.masked_fill_(seq_SP == C.SEQUENCE_CHAINBREAK_TOKEN, C.STRUCTURE_CHAINBREAK_TOKEN)
        ...
        structure_per_res_plddt = enc.structure_per_res_plddt_projection(rbf_16_fn(per_res_plddt))
        ...
        return (
            plddt_embed + structure_per_res_plddt + structure_embed
    ...
    def differentiable_embedding(self, ohe_seq_SPT: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if coordinates is not None:
            ...
            affine, affine_mask = build_affine3d_from_coordinates(coordinates[..., :3, :])
        else:
